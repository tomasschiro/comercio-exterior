...
export default function TablaOperaciones({ userEmail, userId }: Props) {
      ...
      return (
        <input
          ...
          onBlur={() => {
            ...
            setEditing(prev => {
              if (prev && prev.id === op.id && prev.field === field) { commitEdit(prev); return null }
              return prev
            })
          }}
          style={{ ...CELL_INPUT_STYLE, ...(extraStyle ?? {}), minWidth: inputType === 'date' ? 110 : 60 }}
        />
      )
    }
    ...
    return (
      <div
        ...
        style={{
          ...
          borderRadius: 4, padding: '2px 3px', margin: '0 -3px', cursor: 'text', minHeight: 20,
          border: status ? `1px solid ${borderColor}` : 'none',
          background: bgColor, transition: 'background 80ms', overflow: 'hidden',
          ...(extraStyle ?? {}),
        }}
  ...
  function renderSenasaEstadoCell(op: Operacion) {
    ...
    if (!estado || estado === 'pendiente') {
      ...
      badge = <span style={{ display: 'inline-flex', alignItems: 'center', padding: '1px 5px', borderRadius: 4, fontSize: 11, fontWeight: 500, background: '#FBDDD4', color: '#991B1B' }}>Retenida</span>
    } else if (estado === 'liberada') {
      badge = <span style={{ display: 'inline-flex', alignItems: 'center', padding: '1px 5px', borderRadius: 4, fontSize: 11, fontWeight: 500, background: '#E1F1D6', color: '#166534' }}>Liberada</span>
    } else {
      badge = <span style={{ display: 'inline-flex', alignItems: 'center', padding: '1px 5px', borderRadius: 4, fontSize: 11, fontWeight: 500, background: '#E0E4FB', color: '#3730A3' }}>Vinc.{vinculacion ? ` ${formatDateShort(vinculacion)}` : ''}</span>
    }
  ...
  function renderCanalCell(op: Operacion) {
    ...
    if (!canal) {
      ...
      badge = <span style={{ display: 'inline-flex', alignItems: 'center', padding: '1px 7px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: '#E1F1D6', color: '#166534' }}>V</span>
    } else if (canal === 'R') {
      badge = <span style={{ display: 'inline-flex', alignItems: 'center', padding: '1px 7px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: '#FBDDD4', color: '#991B1B' }}>R</span>
    } else {
      badge = <span style={{ display: 'inline-flex', alignItems: 'center', padding: '1px 7px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: '#FDE6CB', color: '#9A3412' }}>N</span>
    }
    return (
      <div
  ...
  const stickyEstado: React.CSSProperties = { position: 'sticky', right: 36, zIndex: 1 }
  const stickyLapiz: React.CSSProperties  = { position: 'sticky', right: 0,  zIndex: 1 }

  return (
    <>
      <style>{`
        .cell-hover:hover { background: rgba(31,27,20,.05); }
        .table-row-hover:hover { background: #FBF7E8; }
        .table-row-hover:hover .edit-btn { opacity: 1 !important; }
        .edit-btn:hover { background: rgba(31,27,20,.07) !important; color: #1F1B14 !important; }
        ...
        .table-row-hover:hover .sticky-estado { background: #FBF7E8; }
        .table-row-hover:hover .sticky-lapiz  { background: #FBF7E8; }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div>
              <h1 style={{ fontSize: 14, fontWeight: 600, color: '#1F1B14', margin: 0, letterSpacing: '-0.01em' }}>
                Operaciones
              </h1>
              <p style={{ fontSize: 12, color: '#7A7158', margin: '2px 0 0' }}>
              ...
              {(['mis', 'todas'] as Tab[]).map(t => (
                <button
                  ...
                  style={{
                    ...
                    color: tab === t ? '#1F1B14' : '#4A4332',
                    boxShadow: tab === t ? '0 1px 2px rgba(31,27,20,.08)' : 'none',
                  }}
                >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ position: 'relative' }}>
              <svg style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', width: 14, height: 14, color: '#7A7158', pointerEvents: 'none' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 14px', height: 32, fontSize: 13, fontWeight: 500, color: '#FFFFFF', background: '#1F1B14', border: 'none', borderRadius: 6, cursor: 'pointer', transition: 'background 120ms', whiteSpace: 'nowrap' }}
        ...
        <div style={{ border: '1px solid #E8DFC5', borderRadius: 10, boxShadow: '0 1px 0 rgba(31,27,20,.04), 0 1px 2px rgba(31,27,20,.05)', overflow: 'hidden', background: '#FFFFFF' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, color: '#1F1B14', tableLayout: 'fixed' }}>
              <colgroup>
                <col style={{ width: 60 }} />
                <col style={{ width: 80 }} />
                <col style={{ width: 100 }} />
                ...
                {loading ? (
                    ...
                    <td colSpan={totalCols} style={{ padding: '64px 16px', textAlign: 'center' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                        ...
                        {!busqueda && (
                          <button
                            onClick={() => setModalOpen(true)}
                            style={{ marginTop: 4, fontSize: 13, color: '#1F1B14', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: 2 }}
                          >
                            + Nueva operación
                          </button>
                        )}
                  ...
                  filtradas.map(op => {
                    ...
                    const estadoStyle: React.CSSProperties = estado === 'Liberado'
                      ? { background: '#E1F1D6', color: '#166534' }
                      : estado === 'En proceso'
                      ? { background: '#D6E1FB', color: '#1E40AF' }
                      : { background: '#FCEBC4', color: '#92400E' }

                    return (
                      <tr key={op.id} className="table-row-hover" style={{ borderBottom: '1px solid #E8DFC5', height: 36 }}>
                        {/* Interno + progress */}
                        <td style={{ padding: '0 4px', overflow: 'hidden', textAlign: 'center', color: '#1D4ED8' }}>
                          <div style={{ fontWeight: 600, ...MONO_STYLE }}>
                            {renderCell(op, 'interno', MONO_STYLE)}
                          </div>
                          <div style={{ height: 2, width: '100%', background: '#E8DFC5', borderRadius: 99, marginTop: 2 }} title={`${done} de 10 completados`}>
                            <div style={{ height: '100%', borderRadius: 99, width: `${pct}%`, background: pct === 100 ? '#166534' : pct > 0 ? '#1E40AF' : 'transparent', transition: 'width 400ms' }} />
                          </div>
                        </td>
                        ...
                        <td style={{ padding: '0 4px', overflow: 'hidden', textAlign: 'center', ...MONO_STYLE, color: '#7A7158' }}>
          ...
          <div style={{ position: 'fixed', top: senasaPopover.top, left: senasaPopover.left, zIndex: 50, background: '#FFFFFF', border: '1px solid #E8DFC5', borderRadius: 8, boxShadow: '0 4px 16px rgba(31,27,20,.10)', padding: 6, width: 168 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8, paddingTop: 8, borderTop: '1px solid #E8DFC5' }}>
              <button
                ...
                style={{ padding: '5px 12px', fontSize: 12, fontWeight: 500, color: '#FFFFFF', background: '#1F1B14', border: 'none', borderRadius: 4, cursor: 'pointer', transition: 'background 100ms' }}
                onMouseEnter={e => { e.currentTarget.style.background = '#111008' }}
                onMouseLeave={e => { e.currentTarget.style.background = '#1F1B14' }}
              >
                Guardar
      ...
      {canalPopover && (
        <>
          ...
          <div style={{ position: 'fixed', top: canalPopover.top, left: canalPopover.left, zIndex: 50, background: '#FFFFFF', border: '1px solid #E8DFC5', borderRadius: 8, boxShadow: '0 4px 16px rgba(31,27,20,.10)', padding: 6, width: 100 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
